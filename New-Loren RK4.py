...
class Particle:
	...
	def acceleration(self, rPos, vPos):
		...
		if iteration == final:
			...
			return q_by_m*np.array(np.cross(vPos, self.bField))/lorentz(vPos) - vPos*(np.linalg.norm(vPos))*((lorentz(vPos)/c)**2)
			...
			if rPos[1]<0:
				...
				return q_by_m*self.eField/lorentz(vPos) - vPos*(np.linalg.norm(vPos))*((lorentz(vPos)/c)**2)
			else:
...
a = 0.0
b = 100*2*math.pi * proton.pMass / (np.linalg.norm(b) * proton.pCharge)
...
print "initial velocity is: ", np.linalg.norm(v[1])
print "final velocity is: ", np.linalg.norm(v[-1])
"""
